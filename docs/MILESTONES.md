...
| P2-M027 | active | Real-agent bridge and dogfooding | A real coding agent completes an AgentForge milestone through `forge task launch`, with path boundaries enforced before commit and repository gates on the agent's commit. |
| P2-M028 | active | Hermetic repository gate under hooks and in worktrees | The gate clears hook Git variables before cargo steps and builds linked worktrees into their own target directory, proven by reproducing the incident before and after the fix. |
